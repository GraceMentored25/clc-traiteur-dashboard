...
const DishRow = memo(function DishRow({ dish }: { dish: Dish }) {
  ...
  const increment = useCallback(() => {
    ...
    else { const next = quantity + 1; setQuantity(next); addToCart({ dish, quantity: next }); }
  ...
  return (
    <div className={cn(
      "grid grid-cols-[2fr_1fr_120px_140px] gap-4 items-center px-4 py-2.5 rounded-xl transition-all",
      inCart ? "bg-[var(--amber)]/5 border border-[var(--amber)]/20" : "hover:bg-[var(--surface-2)] border border-transparent"
    )}>
      {/* Name + image */}
      ...
      {/* Price */}
      <div>
        <span className="text-sm font-bold font-mono text-[var(--amber)]">{formatCurrency(dish.price)}</span>
      ...
      <div className="grid grid-cols-[26px_1fr_26px] items-center gap-1">
        <button
          ...
          className={cn(
            "w-[26px] h-7 rounded-lg flex items-center justify-center transition-all active:scale-95",
            displayQty > 0 ? "bg-[var(--amber)]/15 text-[var(--amber)] hover:bg-[var(--amber)]/25" : "bg-[var(--surface-3)] text-[var(--text-muted)] opacity-40 cursor-not-allowed"
          )}
        >
        ...
        <button
          onClick={increment}
          className="w-[26px] h-7 rounded-lg bg-[var(--amber)] hover:bg-[var(--amber-light)] flex items-center justify-center text-[var(--surface)] transition-all active:scale-95"
        >
