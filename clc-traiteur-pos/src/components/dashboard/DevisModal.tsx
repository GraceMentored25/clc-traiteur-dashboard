...
export default function DevisModal({ onClose }: Props) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    const items: DevisItem[] = cart.map((c) => ({
      dishId: c.dish.id,
          ...
          {success ? (
            <div className="flex flex-col items-center justify-center px-8 py-12 text-center">
              ...
              <div className="flex gap-3 w-full">
                <button
                  onClick={onClose}
                  className="flex-1 h-10 rounded-xl border border-[var(--border)] text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--surface-2)] transition-colors"
                >
                  Nouveau devis
                </button>
                <m.button
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex-1 overflow-y-auto">
              <div className="px-6 py-5 space-y-4">
                ...
                <div className="p-4 rounded-xl bg-[var(--amber)]/8 border border-[var(--amber)]/20">
                  ...
                  {cart.length > 3 && (
                    <p className="text-xs text-[var(--text-muted)] mt-1">
                      +{cart.length - 3} autre{cart.length - 3 > 1 ? "s" : ""} plat{cart.length - 3 > 1 ? "s" : ""}
                    </p>
                  )}
                  <div className="flex justify-between font-bold text-sm text-[var(--amber)] pt-2 mt-1 border-t border-[var(--amber)]/20">
                    <span>Total TTC</span>
                    <span className="font-mono">{formatCurrency(total * 1.2)}</span>
                  </div>
                </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    {errors.clientName && (
                      <p className="text-[11px] text-[var(--danger)] flex items-center gap-1">
                        <WarningCircle size={11} /> {errors.clientName.message}
                      </p>
                    )}
                  </div>
                  <div className="space-y-1.5">
                    ...
                    {errors.clientPhone && (
                      <p className="text-[11px] text-[var(--danger)] flex items-center gap-1">
                        <WarningCircle size={11} /> {errors.clientPhone.message}
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    {errors.eventDate && (
                      <p className="text-[11px] text-[var(--danger)] flex items-center gap-1">
                        <WarningCircle size={11} /> {errors.eventDate.message}
                      </p>
                    )}
                  </div>
                  <div className="space-y-1.5">
                    ...
                    {errors.guestCount && (
                      <p className="text-[11px] text-[var(--danger)] flex items-center gap-1">
                        <WarningCircle size={11} /> {errors.guestCount.message}
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    className="w-full px-3 py-2 rounded-xl bg-[var(--surface-2)] border border-[var(--border)] text-sm text-[var(--text-primary)] placeholder:text-[var(--text-muted)] outline-none focus:border-[var(--amber)]/50 focus:ring-1 focus:ring-[var(--amber)]/15 transition-all resize-none"
                  />
                </div>
              </div>
              ...
              <div className="px-6 py-4 border-t border-[var(--border)] flex gap-3">
                ...
                >
                  {isSubmitting ? (
                    <>
                      <span className="w-3.5 h-3.5 rounded-full border-2 border-[var(--surface)]/30 border-t-[var(--surface)] animate-spin" />
                      Génération...
                    </>
                  ) : (
                    "Créer le devis"
